...
import { compressImageFile, generateFilePath } from "./fileUtil.ts";
...
async function getOssStaToken () {
  ...
  if (result) {
    ...
    compressionSize = result.compressionSize;
...
export function checkFile (file: File,uploadOpt: OssUploadOption = defaultOssUploadOption){
 ...
 * 上传文件到OSS
 */
export async function uploadFile (file: File,uploadOpt: OssUploadOption = defaultOssUploadOption) {
  try {
    // 压缩图片 (如果需要)
    let fileToUpload = file;
    if (file.size >= compressionSize && file.type.startsWith('image/')) {
