...
import { ClassifyManageEnum } from "@/enums/permission/classifyManage.ts";
...
<template>
  <div class="box">
    <el-card body-style="height: 90%;" class="box-card card-left" shadow="hover">
      ...
      <el-form v-permission="ClassifyManageEnum.List" :model="queryParams" :inline="false" @submit.prevent>
      ...
      <div class="ca-btn">
        <el-button v-permission="ClassifyManageEnum.Save" @click="handleAddClassify()"
                   type="primary">添加图书馆
        </el-button>
      </div>
      <el-tree
        ...
        <template #default="{ node, data }">
          ...
          >
            ...
            <div v-if="hoverNodeId === data.id && node.level < 3" class="icon-actions">
              <el-icon v-permission="ClassifyManageEnum.Save" :size="19" color="#409eff" class="icon-item"
                       @click.stop="handleAddCatalog(data)">
                <i-ep-circle-plus/>
              </el-icon>
              <el-icon v-permission="ClassifyManageEnum.Delete" :size="19" color="#f56c6c" class="icon-item"
    ...
    <el-card body-style="height: 90%;overflow:auto;" class="box-card card-right" shadow="hover">
      ...
      <el-form v-if="showStatus && classifyFlag" ref="classifyFormRef" :model="classifyParams"
               :inline="false">
        <el-form-item label="名称" prop="name">
          <el-input v-model="classifyParams.name" clearable class="wid-200"/>
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="classifyParams.description" type="textarea" :rows="5"
                    clearable></el-input>
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="classifyParams.sort" controls-position="right" :min="0"
                           class="wid-200"/>
        </el-form-item>
        ...
        <div class="btn-group-box">
          <div class="btn-box">
            <div>
              <el-button v-permission="ClassifyManageEnum.Save" type="primary" plain @click="saveClassify">完成编辑</el-button>
            </div>
          </div>
        </div>
      </el-form>

      <el-form v-if="showStatus && !classifyFlag" ref="catalogFormRef" :model="catalogParams"
               :inline="false">
        <el-form-item label="名称" prop="name">
          <el-input v-model="catalogParams.name" clearable class="wid-200"/>
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="catalogParams.sort" controls-position="right" :min="0"
                           class="wid-200"/>
        </el-form-item>
        <div class="btn-group-box">
          <div class="btn-box">
            <div>
              <el-button v-permission="ClassifyManageEnum.Save" type="primary" plain @click="updateCatalog">完成编辑</el-button>
            </div>
          </div>
        </div>
      </el-form>
