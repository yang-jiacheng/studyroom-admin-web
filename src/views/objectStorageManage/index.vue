...
import {
  deleteObjectStorage,
  getObjectStoragePageList,
  saveObjectStorage
} from "@/api/objectStorage/index.ts";
...
<template>
  <div>
      ...
      >
        ...
        <el-table-column label="下载地址" prop="downloadUrl" min-width="300">
          <template #default="scope">
            <span class="down-a" @click="handleDownloadUrl(scope.row)" v-if="scope.row.downloadUrl">{{
    ...
    <el-dialog :close-on-click-modal="false" :destroy-on-close="true" v-model="dialogExport.visible"
               :before-close="cancelExportDialog" width="500">
      <template #header>
      ...
      <div style="margin: 20px 0;">
        ...
        >
          <template #tip>
            <span>文件大小不能超过5GB，仅支持单文件上传</span>
          </template>
        </DragUploader>
      </div>
      <template #footer>
